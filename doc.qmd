...
if not path.exists() or not os.environ.get("YNAB_DEV"):
    ...
    transactions_raw = transactions_response.json()["data"]["transactions"]

    # replace empty strings with None so Polars can infer a consistent schema
    sanitized = [{k: (None if v == "" else v) for k, v in rec.items()} for rec in transactions_raw]

    transactions = (
        pl.DataFrame(sanitized)
        ...
        .with_columns(
            ...
            (pl.col("amount").cast(pl.Int64) / 1_000).alias("amount"),
        )
...
# Flag monthly outliers per category using a robust z-score (median + MAD).
# Zero-dollar months are excluded entirely from the calculation (they don't
# count as "typical" spending, low or otherwise) so they never influence the
# median/MAD or get colored themselves.
#
# The color domain starts at the category's median (not zero), so only
# months that spend *more than typical* pick up any color at all. A month
# is fully red once it reaches `outlier_threshold` scaled MADs above the
# median; anything at or below the median stays white.
month_columns = pl.selectors.numeric() - pl.selectors.contains("Total")
outlier_threshold = 3.5  # standard "modified z-score" cutoff

for i in range(0, category_monthly_summary_for_gt.shape[0]):
    row_values = np.array(
        category_monthly_summary_for_gt[i].select(month_columns).row(0)
    )
    nonzero_values = row_values[row_values > 0]

    if nonzero_values.size == 0:
        # No spending in any month for this category/row; nothing to color.
        continue

    median_value = np.median(nonzero_values)
    mad = np.median(np.abs(nonzero_values - median_value))
    mad_scaled = 1.4826 * mad  # normal-consistent scaling of MAD

    if mad_scaled == 0:
        # No spread to compare against (e.g. a single non-zero month, or
        # identical spending every month like Rent at $4,200/mo). There's no
        # basis to call anything an outlier, so leave the row uncolored.
        continue

    domain_min = median_value
    # `data_color()` maps anything outside the domain to `na_color` rather
    # than clamping it to the palette's end color, so genuine outliers above
    # our computed cutoff would otherwise render as invisible white instead
    # of red. Widen `domain_max` to always cover the row's actual max so the
    # most extreme month is guaranteed to fall inside the domain.
    domain_max = max(
        median_value + outlier_threshold * mad_scaled,
        nonzero_values.max(),
    )
...
for i in category_summary.select("group_name", "category_name").to_dicts():
    ...
    base = (
        ...
        .Chart(df)
        .encode(
            x=alt.X("yearmonth(month):O", axis=alt.Axis(format='%b %Y', labelAngle=90), title="Month"),
